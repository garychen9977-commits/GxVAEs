...
class DecoderRNN(nn.Module):
    ...
    def sample_with_logprobs(
        ...
        """
        Core RL rollout method: Samples SMILES tokens and 
        accumulates their log-probabilities for PPO.
        """
        batch_size = z.size(0)
        device = z.device
        
        # Initialize output tensors
        generated_tokens = torch.zeros(batch_size, max_len, dtype=torch.long, device=device)
        log_probs = torch.zeros(batch_size, device=device)
        
        # Start with the '^' (start) token for each molecule in the batch
        input_token = torch.ones([batch_size, 1], dtype=torch.long, device=device) * tokenizer.char_to_int[tokenizer.start]
        
        # Combine latent vector z and gene expression condition
        decode_embed = torch.cat([z, condition], 1)
        # Initialize GRU hidden state from the combined embedding
        hidden = self.i2h(decode_embed).unsqueeze(0).repeat(self.num_layers, 1, 1)

        for i in range(max_len):
            # 1. Forward pass through the GRU step
            output, hidden = self.step(decode_embed, input_token, hidden)
            
            # 2. Apply temperature and calculate probability distribution
            logits = output / max(temperature, 1e-8)
            probs = torch.softmax(logits, dim=1)
            
            # 3. Sample the next token (multinomial sampling ensures exploration)
            next_token = torch.multinomial(probs, 1) # [batch_size, 1]
            
            # 4. Capture the log-probability of the chosen token
            # Use log_softmax for numerical stability in PPO calculations
            step_log_probs = torch.log_softmax(logits, dim=1)
            log_probs += step_log_probs.gather(1, next_token).squeeze(1)
            
            # 5. Store token and set it as the next input
...
class SmilesVAE(nn.Module):
    ...
    def sample_with_logprobs(
        ...
        decode_embed = torch.cat([rand_z, condition], 1)
        hidden = self.decoder.i2h(decode_embed).unsqueeze(0).repeat(self.decoder.num_layers, 1, 1)
    
        for i in range(max_len):
            output, hidden = self.decoder.step(decode_embed, input_token, hidden)
